/*************************************************************************************************/
/*             HW#6 : 문자열                                                                     */
/*  작성자 : 유수형                                날짜 : 2021년 4월 16일                        */
/*                                                                                               */
/* 문제 정의 : 기존에 있던 HW#4의 코드에서 배열로 입력받던 변수들을 getline을 통해               */
/*             문자열 입력을 받고 문자 배열로 입력되던 변수를 모두 string 문자열을               */
/*             통해 받도록 한다.                                                                 */
/*             기존에 있던 ModifyStdInfo함수를 학생정보 뿐만 아니라 수강한 과목의                */
/*             정보까지 수정할 수 있도록 코드를 수정한다.                                        */
/*************************************************************************************************/

#include <iostream>
#include <iomanip>
#include <string.h>
#include<string> // getline정의
#include<cstring>
using namespace std;


struct Subject { // 과목정보
    string SubName; // 과목이름 (string으로 변경)
    int Hakjum; // 과목학점
    string Grade; // 과목등급
    float GPA; // 과목평점
};
...
struct Student { // 학생정보
    string StdName; // 학생이름 (string으로 변경)
    int Hakbun; // 학번
    Subject* Sub; // 과목
...
inline void InputValue(string& str); //문자를 입력받아 자주쓰는 inputvalue들 (과목명, 학색명 등등)
...
Subject* SubSearch(const Student* pSt); // 과목정보 수정에 필요한 search를 위한 함수
...
float CalcAveGPA(Subject* Sub, int StudentNum) //평균 평점을 계산하는 함수
{
    ...
    avg = sum / result; // sum에 대한 값을 result으로 나누어 avg를 구하고
...
{
    int i;
    string name; // 학생 이름을 입력할 변수
    InputValue(name);

    for (i = 0; i < StudentNum; i++)	// 학생 StudentNum명에 대한 정보를 입력받습니다.
    {
        if (name == pSt[i].StdName)			//입력한 name과 StudrntName를 비교해서 0(두개가 같을때)
            return &pSt[i];					//출력
        else
            continue;
    }
...
{
    Student* Searchstd = StdSearch(pSt, StudentNum); // 검색을 위해 입력받은 학생의 이름을 찾습니다.

    if (Searchstd != NULL)
    {
        string Type; // "학생정보" 또는 "과목정보" 문자열 저장용 string
        cout << "수정(학생정보/과목정보):";
        InputValue(Type); // Type값 입력
        cout << "\n";
        if (Type == "학생정보") // Type으로 입력받은 문자열이 "학생정보"일 경우
        {
            cout << Searchstd->StdName << Searchstd->Hakbun << "의 정보를 수정하세요\n"; // 입력받은 [학생이름]의 [학번]을 수정
            cout << "이름 : ";
            InputValue(Searchstd->StdName); // 학생의 이름을 넣습니다.
            cout << "\n";
            cout << "학번 :"; 
            InputValue(Searchstd->Hakbun); // 학생의 학번을 넣습니다
        }
        else if (Type == "과목정보") // Type으로 입력받은 문자열이 "과목정보"일 경우
        {
            string Name; // 검색할 과목에 대한 String
            cout << "검색할 과목 이름 : ";
            InputValue(Name); // 수정할 과목의 이름을 입력합니다.
            if (Searchstd->Sub->SubName == Name) // 입력한 과목의 이름과 위에 검색한 학생이 수강한 과목을 비교
            {
                Subject* Sub = SubSearch(Searchstd);

                cout << Searchstd->Sub->SubName << ", 학점 : " << Searchstd->Sub->Hakjum << ", 등급 : " << Searchstd->Sub->Grade << "의 정보를 수정하세요";
                cout << "\n\n";
                cout << "교과목명 : ";
                ...
                {   
                    ...
                    {
                        CalcGPA(pSt[i].Sub[j]); // 입력받은 과목의 등급별 점수 계싼 후
                        pSt[i].Sub[j].GPA = pSt[i].Sub[j].GPA * pSt[i].Sub[j].Hakjum; // 학점 계산

                    }
                    Searchstd->AveGPA = CalcAveGPA(Searchstd->Sub, Searchstd->SubNum); // 평균학점 재계산
                }
            }
            else // 입력받은 과목이 없을 경우
            {
...
inline void InputValue(string& str) // 자주쓰이는 문자열 입력을 inline함수로 구현
{
    getline(cin, str);
}
...
Subject* SubSearch(const Student* pSt)
{
    ...
    for (int i = 0; i < pSt->SubNum; i++) // 과목수 만큼 반복
    {
        if (Sub == pSt->Sub[i].SubName) // 검색한 과목과 수강한 과목의 이름이 같으면
            return &pSt->Sub[i]; // 과목 return
        else continue;
    }
}