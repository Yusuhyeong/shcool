/*************************************************************************************************/
/*             HW#5 : 동적 메모리 할당                                                           */
/*  작성자 : 유수형                                날짜 : 2021년 4월 9일                         */
/*                                                                                               */
/* 문제 정의 : 기존에 있던 HW#4의 코드에서 학생 수를 입력 받음으로써 지정된 학생                 */
/*             수가 아닌 동적 할당을 통해 원하는 수의 만큼 학생을 입력할 수 있다.                */
/*             또한 한 학생이 입력할 과목 수도 동적 할당을 통해 원하는 과목수 만                 */
/*             큼 입력할 수 있다.                                                                */
/*             (Student* stu = new Student[StdNum];,  pSt[i].Sub = new Subject[pSt[i].SubNum];)  */
/*             InputData함수를 통해 학생 정보 입력에 대한 모듈화를 한다.                         */
/*             CalcAveGPA부분에 과목수가 아닌 총점으로 나누는 것 수정.                           */
/*************************************************************************************************/
...
float CalcAveGPA(Subject* Sub, int StudentNum) //평균 평점을 계산하는 함수
{
    ...
    avg = sum / result; // sum에 대한 값을 3으로 나누어 avg를 구하고
